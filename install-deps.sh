...
# end in '~'). An AD-style username like "first.last" would otherwise produce a
# rule that never takes effect. Sanitize any char outside [A-Za-z0-9_-] to '_'
# for the FILENAME only — the username inside the rule keeps its original form.
...
if sudo -n "$OPENCONNECT" --version >/dev/null 2>&1 \
   && sudo -n /sbin/route -n get default >/dev/null 2>&1; then
    ...
    # If the username was sanitized, remove any pre-existing dotted legacy file
    # (sudo was silently ignoring it anyway) so it doesn't linger.
