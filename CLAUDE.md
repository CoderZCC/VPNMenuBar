...
19. **`connect()` must be mutually exclusive, and a TOTP code is single-use.** `VPNController.connect()` is `@MainActor` but suspends several times (dependency probe off-main, the TOTP step wait, `waitForHandshake`), so two callers — an auto-reconnect racing a menu click, seen 95ms apart in a real log — interleave through those suspension points and each spawn an openconnect. `state = .connecting` is set after the first suspension and is therefore useless as a guard; `connectInFlight` is checked and set with no `await` in between, which on the main actor is a genuine mutex. The damage is not just two processes fighting (each `start()` pkills the other): both mint the **same** TOTP code, ocserv accepts it once and rejects the replay with a plain `401` — indistinguishable from a wrong OTP. Retrying within the same 30s window reuses the same code and fails identically, and three such failures trip the fail2ban cooldown. Hence `lastSubmittedTOTPCounter`: a code that was already handed to the gateway is never sent again, the connect waits out the step instead. When reading a log for this, the tell is two `connect() invoked` lines milliseconds apart followed by two `TOTP generated` lines with an identical `counter=`.
    ...
    **The guard must be persisted, not in-memory — an app restart is the common case, not an edge case.** Applying an update is exactly: old process authenticates, user quits it, new process auto-connects ~5s later. That is inside the same 30s TOTP step, and an in-memory guard is blank in the new process, so **every upgrade replays the previous process's code**. Observed 2026-08-21 (v0.2.15 → v0.2.16): connected at 09:21:39 with `counter=59576323`, the new process resubmitted the identical counter at 09:21:47, the second `POST /auth` returned 401, and the next two attempts got `SSL connection failure: Error in the pull function` / `The TLS connection was non-properly terminated` — i.e. fail2ban had already started resetting the TLS handshake, which reads like a network fault and not like an auth problem. Same-counter-across-two-launch-banners is the signature; note the two `launch` lines with *different versions* between the two `TOTP generated` lines. The counter now lives in `UserDefaults` (key `lastSubmittedTOTPCounter`, `NSNumber`-boxed `UInt64`), following the `launchAtLoginEnabled` precedent so `VPNConfig`'s Codable schema is untouched. Note this fix cannot cover its own rollout: the process being replaced never wrote the key, so the first connect after upgrading *to* the fixed build can still replay once — wait one 30s step and retry.
