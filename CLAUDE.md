...
**Filename must not contain a `.`** — sudo silently ignores files in `/etc/sudoers.d/` whose names contain a dot or end in `~` (to skip editor/package backup files). AD-style usernames like `first.last` would otherwise produce a rule that is written correctly, validated by `visudo -c`, and then never loaded — the failure mode is invisible (`sudo -n` keeps asking for a password even though the file looks perfect). Both `install-deps.sh` and `DependencyInstaller.installSudoersRule` sanitize the **filename's** `<user>` component (any char outside `[A-Za-z0-9_-]` → `_`, so `first.last` → `vpnmenubar-first_last`) while leaving the username **inside the rule** untouched (dots are fine there). They also `rm` any pre-existing dotted legacy file. Never reference the file as `vpnmenubar-$(whoami)` — use the `vpnmenubar-*` glob.
